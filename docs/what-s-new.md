...
Note: The _New UI Preview program_ has since then been closed, so there's no way to download the plugin anymore officially. However, we still have a copy of it available [here](http://dl.material-theme.com/newUI). You can also find it in our Slack channel.
